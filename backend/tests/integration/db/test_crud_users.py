...
class TestGetOrCreateUser:
    async def test_creates_new_user_on_first_call(self, db_session: AsyncSession):
        user, created = await crud.get_or_create_user(db_session, clerk_id="google-new-123", email="new@example.com")
    ...
    async def test_returns_existing_user_on_second_call(self, db_session: AsyncSession):
        u1, _ = await crud.get_or_create_user(db_session, clerk_id="google-existing-456", email="a@example.com")
        u2, created = await crud.get_or_create_user(db_session, clerk_id="google-existing-456", email="a@example.com")
    ...
    async def test_updates_email_on_second_call(self, db_session: AsyncSession):
        ...
        user, _ = await crud.get_or_create_user(db_session, clerk_id=gid, email="new@example.com")
        assert user.email == "new@example.com"

    async def test_last_seen_at_updated_on_second_call(self, db_session: AsyncSession):
        gid = f"google-{uuid.uuid4().hex[:8]}"
        u1, _ = await crud.get_or_create_user(db_session, clerk_id=gid, email="a@example.com")
        first_seen = u1.last_seen_at
        u2, _ = await crud.get_or_create_user(db_session, clerk_id=gid, email="a@example.com")
